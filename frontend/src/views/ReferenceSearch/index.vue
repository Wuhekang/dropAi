  ...
  <main class="reference-page">
    <nav class="top-nav page-nav">
      ...
      <div class="step-track" aria-label="纯文字稿生成步骤">
        <span class="step active"><b>1</b> AI 文献搜索</span>
        <i></i>
        <span class="step"><b>2</b> 上传素材</span>
        <i></i>
        <span class="step"><b>3</b> 模式与提纲</span>
        <i></i>
        <span class="step"><b>4</b> 图片绑定</span>
        <i></i>
        <span class="step"><b>5</b> 正文生成</span>
      </div>
    ...
    <template v-else>
      ...
      <footer class="result-actions panel">
        ...
        <div class="action-buttons">
          ...
          <button class="primary-button" type="button" :disabled="saving || !quotaSatisfied" @click="goToOutline">下一步上传素材 →</button>
        </div>
      </footer>
    ...
    <section class="panel history-panel">
      <div class="library-head"><div><span>MY TASKS</span><h2>我的生成任务</h2><p>同一账号可保留多个项目，并分别查看进度或下载结果。</p></div><button class="ghost-button" type="button" @click="loadHistory">刷新</button></div>
      <div v-if="history.length" class="history-list">
        <article v-for="item in history" :key="item.id">
          <div><h3>{{ item.projectName }}</h3><p>{{ item.documentType || '纯文字稿' }} · {{ statusText(item.status) }} · {{ item.createdTime }}</p><small v-if="item.errorMessage">{{ item.errorMessage }}</small></div>
          <el-progress :percentage="Number(item.progress || 0)" :status="item.status==='FAILED'?'exception':item.status==='SUCCESS'?'success':''"/>
          <button class="ghost-button" type="button" @click="openHistory(item)">{{ item.downloadAvailable ? '查看并下载' : item.status==='FAILED' ? '重新生成' : '查看进度' }}</button>
        </article>
      </div>
...
import { addWritingManualReference, createWritingProject, generateWritingV2Outline, getWritingProjectHistory, getWritingReferenceProviders, importWritingReferences, saveWritingReferenceLibrary, startAiReferenceSearch } from '../../api/rewrite'
...
async function loadProviders() {
  ...
  try {
    ...
    providerState.value = 'warning'
    providerMessage.value = '联网服务状态暂时无法检测，提交检索时将再次尝试。'
  }
}
...
async function goToOutline() {
  ...
  router.push('/writing-generator/materials')
}
...
async function loadHistory(){ history.value=await getWritingProjectHistory() }
function statusText(status){return ({WAITING:'等待生成',GENERATING:'生成中',SUCCESS:'生成完成',FAILED:'生成失败'})[status]||status}
function openHistory(item){sessionStorage.setItem('dropai_writing_project_id',item.id);router.push(item.status==='WAITING'?'/writing-generator/materials':'/writing-generator/generate')}
onMounted(async()=>{await loadProviders();await loadHistory()})
...
.history-panel{margin-top:28px;padding:30px}.history-list{display:grid;gap:12px;margin-top:20px}.history-list article{display:grid;grid-template-columns:minmax(260px,1fr) minmax(220px,360px) auto;align-items:center;gap:20px;padding:16px;border:1px solid var(--line-soft);border-radius:12px}.history-list h3,.history-list p{margin:0}.history-list p,.history-list small{color:var(--muted);font-size:12px}.history-list small{color:#c45656}@media(max-width:800px){.history-list article{grid-template-columns:1fr}}
</style>
