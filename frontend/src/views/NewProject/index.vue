<template>
  <main class="workspace">
    <nav class="topbar">
      <button class="brand" @click="router.push('/')"><b>D</b><span>Dokiai Mechanical Studio<small>AI MECHANICAL WORKSPACE</small></span></button>
      <button class="back-link" @click="router.push('/dashboard')">返回工作台</button>
    </nav>

    <header class="header">
      <div><span>DOKIAI MECHANICAL STUDIO</span><h1>AI 机械设计工作台</h1></div>
      <strong :class="statusClass">{{ statusText }}</strong>
    </header>

    <section class="shell">
      <aside class="input-pane studio-card">
        <span class="pane-kicker">REQUIREMENT INPUT</span><h2>设计需求</h2>
      ...
      <section class="main-pane studio-card">
        <div class="process">
        ...
        <div class="model-row">
          ...
          <aside class="summary"><span class="pane-kicker">AI DESIGN ASSISTANT</span>
...
.workspace{padding:18px max(26px,calc((100% - 1540px)/2)) 48px!important;color:#252936!important;background:linear-gradient(45deg,#fbd7ea 0%,#f8edf5 38%,#eef1f8 65%,#dcebff 100%)!important}.brand span{display:grid;text-align:left}.brand small{color:#969cac;font-size:8px}.brand b{border-radius:11px!important;background:linear-gradient(145deg,#4198ff,#7658ef 60%,#df66b7)!important}.header{padding:30px 0 20px!important}.header span,.pane-kicker{color:#6e4fff!important;font-size:9px!important;font-weight:800;letter-spacing:.14em}.header h1{font-size:42px!important}.header>strong{border:1px solid #dcd5ef!important;border-radius:99px!important;background:#ffffffaa!important}.shell{gap:16px!important}.input-pane,.main-pane{border:1px solid rgba(110,79,255,.1)!important;border-radius:22px!important;background:#ffffffdf!important;box-shadow:0 18px 50px rgba(61,53,104,.08)!important}.input-pane{padding:20px!important}.input-pane textarea{border-color:#ddd6ec!important;border-radius:12px!important;background:#faf9fd}.upload{border-color:#cbbfee!important;border-radius:11px!important;background:#faf8ff}.input-pane>button{border-color:#c4b8ed!important;border-radius:10px!important;color:#6e4fff}.input-pane>.primary{border:0!important;background:linear-gradient(135deg,#6e4fff,#ff55b0)!important;color:#fff}.main-pane{padding:15px!important}.process div{border-radius:9px!important;background:#f5f2fb!important}.process .passed i{background:#6e4fff!important}.model-row{gap:14px!important}.model-row> :first-child{overflow:hidden;border-radius:16px}.summary{border:0!important;border-radius:16px;background:#f8f5ff}.tabs{border-color:#ebe6f4!important}.tabs .active{border-color:#6e4fff!important;color:#6e4fff!important}.panel{border-radius:15px;background:#fff}.part li{border-color:#6e4fff!important;background:#f5f1ff!important}.part li i{background:#6e4fff!important}
.topbar{height:62px!important;padding:0 4px}.back-link{color:#6e4fff!important;font-weight:700}.header h1{max-width:760px;line-height:1.08}.shell{grid-template-columns:330px minmax(0,1fr)!important}.studio-card{backdrop-filter:blur(18px)}.input-pane h2{margin:7px 0 14px;font-size:24px}.input-pane textarea{min-height:250px!important;line-height:1.65}.input-pane>button{min-height:42px;font-weight:700}.input-pane>button.primary,.input-pane>button.resume{border:0!important;background:linear-gradient(135deg,#6e4fff,#ff55b0)!important;color:#fff!important;box-shadow:0 10px 24px rgba(110,79,255,.18)}.tools{border-color:#eee9f5!important}.tools em.ok{color:#6e4fff!important}.process{gap:7px!important}.process div{min-height:46px;align-content:center;padding:10px!important}.process i{border-radius:4px}.process .running i{background:#ff55b0!important}.model-row{grid-template-columns:minmax(0,1fr) 300px!important;margin-top:14px!important}.model-row> :first-child{min-height:470px;background:linear-gradient(145deg,#f5f2fb,#eef5ff)!important}.summary{padding:22px!important;background:linear-gradient(160deg,#f5f1ff,#fff4fa)!important}.summary h2{margin:7px 0 12px;font-size:23px}.summary p{line-height:1.65}.tabs{gap:8px!important;padding-top:8px}.tabs button{border-radius:10px 10px 0 0!important;color:#746e7d}.tabs .active{background:#f5f1ff!important}.panel{margin-top:2px;min-height:220px!important;padding:22px!important;background:rgba(255,255,255,.7)!important}.header>strong.passed{color:#6e4fff!important}.brand b{width:36px!important;height:36px!important;box-shadow:0 8px 20px rgba(110,79,255,.22)}
@media(max-width:1050px){.shell{grid-template-columns:1fr!important}.model-row{grid-template-columns:1fr!important}}
.input-pane>button:not(:disabled){border:0!important;background:linear-gradient(135deg,#6e4fff,#ff55b0)!important;color:#fff!important;box-shadow:0 10px 24px rgba(110,79,255,.14)}
</style>
