...
<script setup>
import { reactive, ref } from 'vue';import { useRouter } from 'vue-router';import { login, register } from '../../api/rewrite'
const router=useRouter(),registering=ref(false),loading=ref(false),form=reactive({phone:'',password:''})
async function submit(){if(loading.value||!form.phone||!form.password)return;loading.value=true;try{const result=await(registering.value?register(form):login(form));sessionStorage.setItem('dropai_token',result.token);sessionStorage.setItem('dropai_username',result.username);sessionStorage.setItem('dropai_role',result.role||'USER');router.replace('/dashboard')}finally{loading.value=false}}
</script>
<style scoped>
