...
<script setup>
import { computed,reactive,ref } from 'vue';import { useRoute,useRouter } from 'vue-router';import { login, register } from '../../api/rewrite'
...
async function submit(){if(loading.value||!form.phone||!form.password)return;loading.value=true;try{const payload=registering.value?{...form,...(college.value?{college:college.value}:{})}:form;const result=await(registering.value?register(payload):login(payload));sessionStorage.setItem('dropai_token',result.token);sessionStorage.setItem('dropai_username',result.username);sessionStorage.setItem('dropai_role',result.role||'USER');if(result.schoolName)sessionStorage.setItem('dropai_school_name',result.schoolName);else sessionStorage.removeItem('dropai_school_name');router.replace(String(result.role).toUpperCase()==='SCHOOL_VIEWER'?'/school-statistics':'/dashboard')}finally{loading.value=false}}
</script>
<style scoped>
