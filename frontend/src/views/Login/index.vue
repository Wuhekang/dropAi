...
const redirect=computed(()=>{const value=typeof route.query.redirect==='string'?route.query.redirect:'';return value.startsWith('/')&&!value.startsWith('//')?value:''})
async function submit(){if(loading.value||!form.phone||!form.password)return;loading.value=true;try{const payload=registering.value?{...form,...(college.value?{college:college.value}:{})}:form;const result=await(registering.value?register(payload):login(payload));setAuthSession(result);const schoolViewer=String(result.role).toUpperCase()==='SCHOOL_VIEWER';router.replace(schoolViewer?'/school-statistics':redirect.value||'/dashboard')}finally{loading.value=false}}
</script>
<style scoped>
