...
function openSchoolForm(s){
  ...
  const ownPrice=Number.isFinite(configuredOwnPrice)?Math.max(0.3,configuredOwnPrice):0.3
  const minimumPrice=Number.isFinite(configuredMinimumPrice)?Math.max(0.3,configuredMinimumPrice):1
  const configuredStudentPrice=s?Number(s.studentRechargePricePer10??2):2
  const studentPrice=Math.max(0.3,ownPrice,minimumPrice,Number.isFinite(configuredStudentPrice)?configuredStudentPrice:2)
  Object.assign(schoolForm,s?{id:s.id,schoolName:s.schoolName,schoolCode:s.schoolCode,rechargePricePer10:ownPrice,studentRechargeMinPricePer10:minimumPrice,studentRechargePricePer10:studentPrice,enabled:s.enabled}:{id:null,schoolName:'',schoolCode:'',rechargePricePer10:0.3,studentRechargeMinPricePer10:1,studentRechargePricePer10:2,enabled:true})
  schoolFormOpen.value=true
  menuId.value=null
}
async function saveSchool(){
  if(schoolCodeError.value||!schoolForm.schoolName)return
  const ownPrice=Number(schoolForm.rechargePricePer10),minimumPrice=Number(schoolForm.studentRechargeMinPricePer10),price=Number(schoolForm.studentRechargePricePer10)
  ...
  if(!Number.isFinite(minimumPrice)||minimumPrice<0.3||minimumPrice>1000||Math.abs(minimumPrice*100-Math.round(minimumPrice*100))>1e-7)return ElMessage.warning('下级账号最低充值价须为 0.30–1000 元，且最多两位小数')
  ...
  const payload={schoolName:schoolForm.schoolName,schoolCode:schoolForm.schoolCode,rechargePricePer10:ownPrice,studentRechargeMinPricePer10:minimumPrice,studentRechargePricePer10:price,enabled:schoolForm.enabled}
  saving.value=true
...
.pricing-cell{display:grid;gap:3px;min-width:0}.pricing-cell b,.pricing-cell small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pricing-cell small{color:#8d94a4;font-size:9px;font-weight:500}
</style>
<style scoped>
