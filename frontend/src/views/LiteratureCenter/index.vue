<template>
  <main class="literature-page">
    ...
    <section v-if="result" class="summary"><div><span>{{ isPartial?'PARTIAL RESULTS':'SEARCH COMPLETE' }}</span><h2>共找到 {{ actualCount }} 篇文献</h2><p>中文目标 {{ result.chineseCount }} 篇 · 英文目标 {{ result.englishCount }} 篇 · 实际按 {{ result.unitCostPoints }} 积分/篇扣除 {{ result.costPoints }} 积分</p><p v-if="warningText" class="partial-note">{{ warningText }}</p></div><button @click="copyCitation">复制全部 GB/T 7714</button></section>
...
const form = ref({ title: '', chineseCount: 5, englishCount: 5 })
...
const countValidationMessage = computed(() => {
  ...
  if (totalCount.value < 1) return '中文和英文文献数量不能同时为 0'
...
const billingHint = computed(() => {
  ...
  return `单次最多检索 20 篇 · ${priceText} · 提交前按目标数量校验余额，完成后仅按实际返回结果扣费`
})
const items = computed(() => result.value?.items || [])
const requestedCount = computed(() => Number(result.value?.requestedCount ?? totalCount.value))
const actualCount = computed(() => Number(result.value?.actualCount ?? items.value.length))
const isPartial = computed(() => Boolean(result.value?.partial) || actualCount.value < requestedCount.value)
const warningText = computed(() => {
  const warnings = Array.isArray(result.value?.warnings)
    ? result.value.warnings.map(warning => warning?.message || String(warning)).filter(Boolean)
    : []
  return warnings.join('；') || (isPartial.value ? '部分公开来源暂未返回结果，已展示成功检索到的文献。' : '')
})
...
async function submit() {
  ...
  try {
    result.value = await searchLiterature({ title: form.value.title.trim(), ...counts })
    const rawResponseUnitCost = result.value?.unitCostPoints
    ...
    if (isPartial.value) {
      ElMessage.warning({ message: `已返回 ${actualCount.value}/${requestedCount.value} 篇，部分来源超时或暂无结果；仅按实际结果扣费。`, duration: 6000 })
    } else {
      ElMessage.success(`文献搜索完成：${items.value.length} 篇`)
    }
  } finally {
