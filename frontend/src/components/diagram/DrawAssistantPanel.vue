  ...
  <section class="draw-assistant" :data-stage="stage">
    <div class="visual" aria-live="polite">
      <picture :key="stage" class="character-picture">
        <source v-if="!reduceMotion" :srcset="asset('animations/webp', 'webp')" type="image/webp">
        <img class="character" :src="reduceMotion ? asset('characters/pipeline', 'png') : asset('animations/gif', 'gif')" :alt="`原创绘图助手：${label}`" draggable="false" @error="useStaticFallback">
      </picture>
...
const reduceMotion=ref(false);let media
...
function useStaticFallback(event){const fallback=asset('characters/pipeline','png');if(!event.currentTarget.src.endsWith(fallback))event.currentTarget.src=fallback}
...
function updateMotion(event){reduceMotion.value=event.matches}
onMounted(()=>{media=window.matchMedia('(prefers-reduced-motion: reduce)');updateMotion(media);media.addEventListener?.('change',updateMotion);['idle','analyzing','generating','assembling','rendering','success','error'].forEach(stage=>{const image=new Image();image.src=`/draw-assistant/animations/webp/assistant_${stage}.webp`})})
onBeforeUnmount(()=>media?.removeEventListener?.('change',updateMotion))
</script>

<style scoped>
.draw-assistant{--violet:#8b5cf6;--pink:#f09de8;--blue:#7dd3fc;display:grid;grid-template-rows:minmax(118px,1fr) auto auto auto;gap:8px;height:100%;min-height:0;padding:10px 12px;background:url('/draw-assistant/backgrounds/assistant-card-bg.svg') center/cover;border-radius:0 0 16px 16px;color:#17213a}.visual{position:relative;min-height:118px;overflow:hidden;border-radius:15px;background:linear-gradient(135deg,#ffffff8c,#f5f0ff82)}.character-picture{position:absolute;inset:auto auto -18px 0;width:52%;height:138%;display:flex;align-items:flex-end;justify-content:center;animation:pop .28s ease}.character{display:block;max-width:100%;max-height:100%;object-fit:contain;pointer-events:none;user-select:none;filter:drop-shadow(0 12px 18px #5947aa24);animation:float 2.8s ease-in-out infinite}.status-bubble{position:absolute;top:18px;right:14px;display:flex;align-items:center;gap:8px;max-width:61%;min-height:42px;padding:9px 13px;border:1px solid #a78bfa38;border-radius:999px;background:#fffffff0;box-shadow:0 8px 22px #5947aa19;color:var(--violet);font-size:12px;font-weight:700}.status-bubble>img{width:24px;height:24px}.status-bubble>span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dots{display:flex;gap:3px}.dots b{width:4px;height:4px;border-radius:50%;background:#a78bfa;animation:dot 1.1s ease-in-out infinite}.dots b:nth-child(2){animation-delay:.14s}.dots b:nth-child(3){animation-delay:.28s}.progress{display:grid;grid-template-columns:repeat(4,minmax(70px,1fr));gap:4px;overflow-x:auto;color:#9a9fba;font-size:11px}.progress span{position:relative;padding:5px 2px 7px;text-align:center;white-space:nowrap}.progress span:after{content:'';position:absolute;right:13%;bottom:0;left:13%;height:2px;border-radius:2px;background:#e6e1f7}.progress .active{color:var(--violet);font-weight:700}.progress .active:after,.progress .done:after{background:linear-gradient(90deg,var(--blue),#a78bfa,var(--pink))}.progress .done{color:#64708f}.progress .error{color:#f27193}.input-shell{display:grid;grid-template-columns:minmax(0,1fr) 42px;align-items:center;gap:8px;padding:7px 8px 7px 14px;border:1px solid #a78bfa42;border-radius:18px;background:#fffffff2;box-shadow:0 8px 22px #5947aa14}.input-shell textarea{min-width:0;resize:none;border:0;outline:0;background:transparent;color:#28324c;font:12px/1.45 'Microsoft YaHei',sans-serif}.send{display:grid;place-items:center;width:42px;height:42px;padding:0;border:0;border-radius:14px;background:linear-gradient(135deg,var(--blue),#a78bfa,var(--pink));box-shadow:0 7px 16px #8b5cf647;color:#fff}.send img{width:42px;height:42px}.send.stop{font-size:15px}.details{min-height:28px}.details summary{display:flex;align-items:center;justify-content:center;gap:6px;color:#7882a0;font-size:11px;cursor:pointer;list-style:none}.details summary img{width:18px;height:18px}.details[open]{overflow:auto;max-height:145px;padding-top:4px}.detail-actions{display:flex;gap:5px;overflow-x:auto;padding:4px 0}.detail-actions button,.details footer button{padding:4px 7px;font-size:10px}.messages{display:grid;gap:5px}.messages p{margin:0;padding:6px 8px;border-radius:8px;background:#ffffffb5;font-size:10px;white-space:pre-wrap}.messages p.user{background:#ede9ffcc}.messages b{display:block;color:var(--violet)}.details footer{display:flex;justify-content:flex-end;gap:5px;margin-top:5px}@keyframes float{50%{transform:translateY(-5px)}}@keyframes dot{50%{opacity:.35;transform:translateY(-2px)}}@keyframes pop{from{opacity:0;transform:translateY(5px) scale(.97)}}@media(prefers-reduced-motion:reduce){.character,.character-picture,.dots b{animation:none!important}}@media(max-height:800px){.visual{min-height:96px}.draw-assistant{grid-template-rows:minmax(96px,1fr) auto auto auto}.status-bubble{top:10px}}button:disabled{cursor:not-allowed;opacity:.5}
</style>
