...
function rewritePage(job) {
  ...
  const context = vm.createContext({
    ...
    ElMessage: { success: noop, warning: noop, error: noop }, ElMessageBox: {},
    ...
    document: { createElement: () => ({ click: () => { counts.clicks += 1 } }) }
  })
  vm.runInContext(`${source}\nglobalThis.page = { startDocumentPolling, handleDocumentAction, downloadDocumentJob, setDocumentJob, documentJob, docProgress, docActionDisabled, docActionText, taskStateLabel };`, context)
  return { page: context.page, counts }
}
