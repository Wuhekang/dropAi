...
SET @dokiai_ddl = IF(
  ...
  'SELECT 1',
  'ALTER TABLE school ADD COLUMN student_recharge_price_per10 DECIMAL(10,2) NOT NULL DEFAULT 0.30 COMMENT ''下级注册账号兑换10积分所需金额'''
);
PREPARE dokiai_stmt FROM @dokiai_ddl; EXECUTE dokiai_stmt; DEALLOCATE PREPARE dokiai_stmt;
...
UPDATE school
SET student_recharge_price_per10 = GREATEST(
  COALESCE(student_recharge_price_per10, 0.30),
  COALESCE(recharge_price_per10, 0.30),
  0.30
);
