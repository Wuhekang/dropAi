...
public class DoubaoDiagramClient {
    ...
    private final DiagramAssistantProperties properties; private final ObjectMapper mapper;
    public DoubaoDiagramClient(DiagramAssistantProperties properties,ObjectMapper mapper){this.properties=properties;this.mapper=mapper;}
    public ModelResult generate(DiagramType type,DiagramPromptFactory.Prompt prompt,IntConsumer onDelta){
        ...
        try{
            ...
            HttpURLConnection connection=open(mapper.writeValueAsString(body),key,properties.getFirstByteTimeout());
            int status;try{status=connection.getResponseCode();}catch(SocketTimeoutException e){connection.disconnect();throw new DiagramGenerationException("MODEL_FIRST_BYTE_TIMEOUT","豆包20秒内未建立响应，本次生成已终止，原图已恢复。");}
            InputStream response=body(connection,status);if(status/100!=2){String text=new String(response.readNBytes(1000),StandardCharsets.UTF_8);connection.disconnect();throw new DiagramGenerationException("DOUBAO_HTTP_ERROR","豆包请求失败（HTTP "+status+")："+safe(text));}
            try{return consume(response,started,onDelta);}finally{connection.disconnect();}
        }catch(DiagramGenerationException e){throw e;}catch(SocketTimeoutException e){throw new DiagramGenerationException("DOUBAO_CONNECT_TIMEOUT","连接豆包超过5秒，本次生成已终止，原图已恢复。");}
    ...
    public SummaryResult summarize(String source,int maxChars){
        ...
        try{
            ...
            HttpURLConnection connection=open(mapper.writeValueAsString(body),key,properties.getHardLimit());int status=connection.getResponseCode();String response;
            try(InputStream input=body(connection,status)){response=new String(input.readAllBytes(),StandardCharsets.UTF_8);}finally{connection.disconnect();}
            if(status/100!=2)throw new DiagramGenerationException("DOUBAO_SUMMARY_FAILED","长文本压缩失败（HTTP "+status+"），尚未生成图形。");
            JsonNode root=mapper.readTree(response);String content=root.path("choices").path(0).path("message").path("content").asText("");String summary=mapper.readTree(content).path("summary").asText("").trim();
    ...
    private HttpURLConnection open(String json,String key,Duration readTimeout)throws IOException{
        HttpURLConnection connection=(HttpURLConnection)new URL(properties.getEndpoint()).openConnection();connection.setRequestMethod("POST");connection.setDoOutput(true);connection.setConnectTimeout((int)Math.min(Integer.MAX_VALUE,properties.getConnectTimeout().toMillis()));connection.setReadTimeout((int)Math.min(Integer.MAX_VALUE,readTimeout.toMillis()));connection.setRequestProperty("Authorization","Bearer "+key);connection.setRequestProperty("Content-Type","application/json");byte[] bytes=json.getBytes(StandardCharsets.UTF_8);connection.setFixedLengthStreamingMode(bytes.length);try(OutputStream output=connection.getOutputStream()){output.write(bytes);}return connection;
    }
    private static InputStream body(HttpURLConnection connection,int status)throws IOException{InputStream input=status>=400?connection.getErrorStream():connection.getInputStream();return input==null?InputStream.nullInputStream():input;}
