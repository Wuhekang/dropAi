...
public class SemanticIrLiteAdapter {
    ...
    private ErDiagramIr er(List<JsonNode> nodes,List<JsonNode> relations,String title){
        ...
        for(JsonNode n:entityNodes){List<ErAttribute> attrs=new ArrayList<>();int index=0;for(JsonNode a:list(n.path("attributes"))){String name=text(a,"name");if(blank(name))continue;index++;attrs.add(new ErAttribute(ids.get(text(n,"text"))+"A"+index,name,blank(text(a,"type"))?"VARCHAR(255)":text(a,"type"),bool(a,"pk"),bool(a,"fk"),bool(a,"unique"),!a.has("nullable")||bool(a,"nullable")));}if(attrs.isEmpty())attrs.add(new ErAttribute(ids.get(text(n,"text"))+"A1",text(n,"text")+"ID","BIGINT",true,false,true,false));entities.add(new ErEntity(ids.get(text(n,"text")),text(n,"text"),attrs,false));}
