...
public class OllamaDiagramClient {
    ...
    public DoubaoDiagramClient.ModelResult generate(DiagramType type,String instruction,DiagramIr current,IntConsumer onDelta){
        ...
        try{
            ...
            if(http!=null){HttpRequest request=HttpRequest.newBuilder(URI.create(properties.getOllamaEndpoint())).timeout(properties.getOllamaTimeout()).header("Content-Type","application/json").POST(HttpRequest.BodyPublishers.ofString(requestBody,StandardCharsets.UTF_8)).build();HttpResponse<String> response=http.send(request,HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8));status=response.statusCode();responseBody=response.body();}
            else{HttpURLConnection connection=(HttpURLConnection)new URL(properties.getOllamaEndpoint()).openConnection();connection.setRequestMethod("POST");connection.setDoOutput(true);connection.setConnectTimeout((int)Math.min(Integer.MAX_VALUE,Math.min(properties.getConnectTimeout().toMillis(),properties.getOllamaTimeout().toMillis())));connection.setReadTimeout((int)Math.min(Integer.MAX_VALUE,properties.getOllamaTimeout().toMillis()));connection.setRequestProperty("Content-Type","application/json");byte[] bytes=requestBody.getBytes(StandardCharsets.UTF_8);connection.setFixedLengthStreamingMode(bytes.length);try(OutputStream output=connection.getOutputStream()){output.write(bytes);}status=connection.getResponseCode();try(InputStream input=status>=400?connection.getErrorStream():connection.getInputStream()){responseBody=input==null?"":new String(input.readAllBytes(),StandardCharsets.UTF_8);}finally{connection.disconnect();}}
        ...
        }catch(DiagramGenerationException e){throw e;}catch(HttpTimeoutException|java.net.SocketTimeoutException e){throw new DiagramGenerationException("OLLAMA_TIMEOUT","本地绘图模型5秒内未返回完整结果。",true,e);}catch(Exception e){if(Thread.currentThread().isInterrupted())throw new DiagramGenerationException("GENERATION_CANCELLED","生成已取消，原图已恢复。",false,e);throw new DiagramGenerationException("OLLAMA_REQUEST_FAILED","无法连接本地绘图模型，原图已恢复。",true,e);}
    }
