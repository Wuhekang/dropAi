...
public class PptRenderValidatorV1 {
    private static final List<String> FORBIDDEN=List.of("Click to edit Master text styles","Second level","<#>","占位符");
    public Report validate(PptLayoutPlannerV1.LayoutResult tree,Path ppt,long renderTimeMs){int text=0,image=0,overlap=0,placeholder=0;List<String> issues=new ArrayList<>();
        ...
        boolean open=false;try(InputStream in=Files.newInputStream(ppt);XMLSlideShow show=new XMLSlideShow(in)){open=show.getSlides().size()==tree.pageCount();}catch(Exception e){issues.add("PPTX无法打开: "+e.getMessage());}
