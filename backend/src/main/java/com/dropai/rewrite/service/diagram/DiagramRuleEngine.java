...
public class DiagramRuleEngine {
    ...
    static final class UseCaseRuleEngine implements Rule<UseCaseIr>{public UseCaseIr normalize(UseCaseIr x){List<Actor> actors=unique(x.actors(),Actor::id);List<UseCase> cases=unique(x.useCases(),UseCase::id);Set<String> all=new HashSet<>();actors.forEach(a->all.add(a.id()));cases.forEach(u->all.add(u.id()));List<UseCaseRelation> rels=new ArrayList<>();Set<String> seen=new HashSet<>();for(UseCaseRelation r:safe(x.relations())){if(!all.contains(r.from())||!all.contains(r.to()))throw invalid("用例关系引用不存在对象");String key=r.from()+">"+r.to()+":"+r.kind();if(seen.add(key))rels.add(new UseCaseRelation(id(r.id(),"r",rels.size()+1),r.from(),r.to(),r.kind()==null?UseCaseRelationKind.ASSOCIATION:r.kind()));}return new UseCaseIr("1.0",DiagramType.USE_CASE,cleanTitle(x.title(),"用例图"),id(x.systemId(),"S",1),cleanTitle(x.systemName(),x.title()),actors,cases,rels,warnings(x.warnings()));}}
    static final class BlockDiagramRuleEngine implements Rule<BlockDiagramIr>{public BlockDiagramIr normalize(BlockDiagramIr x){List<Block> blocks=unique(x.blocks(),Block::id);Map<String,Block> by=index(blocks,Block::id);return new BlockDiagramIr("1.0",DiagramType.BLOCK_DIAGRAM,cleanTitle(x.title(),"系统框图"),blocks,edges(x.edges(),by.keySet()),warnings(x.warnings()));}}
    static final class SequenceDiagramRuleEngine implements Rule<SequenceDiagramIr>{public SequenceDiagramIr normalize(SequenceDiagramIr x){List<Participant> ps=unique(x.participants(),Participant::id);Map<String,Participant> by=index(ps,Participant::id);List<Message> ms=new ArrayList<>();int order=0;for(Message m:safe(x.messages())){if(!by.containsKey(m.from())||!by.containsKey(m.to()))throw invalid("时序消息引用不存在参与者");if(ms.size()>=8)break;ms.add(new Message(id(m.id(),"m",++order),m.from(),m.to(),m.kind()==null?MessageKind.CALL:m.kind(),m.text(),order));}if(ps.size()<2||ps.size()>6)throw invalid("时序图参与者数量必须为2至6个");if(ms.isEmpty())throw invalid("时序图至少需要一条消息");return new SequenceDiagramIr("1.0",DiagramType.SEQUENCE_DIAGRAM,cleanTitle(x.title(),"时序图"),ps,ms,warnings(x.warnings()));}}
    ...
    private static String id(String value,String prefix,int number){return blank(value)?prefix+number:value.replaceAll("[^A-Za-z0-9_-]","");}private static String cleanTitle(String v,String d){return blank(v)?d:v.trim();}private static String card(String v){return "1".equals(v)?"1":"M".equalsIgnoreCase(v)?"M":"N";}private static String nz(String v){return v==null?"":v;}private static boolean blank(String v){return v==null||v.isBlank();}private static <T> List<T> safe(List<T> v){return v==null?List.of():v;}private static List<Warning> warnings(List<Warning> v){return safe(v);}
