...
public class DoubaoAiRewriteService implements AiRewriteService {
    ...
    private String bodyOnlyProtectionPrompt() {
        return """
                ...
                除正文段落外，以下内容禁止修改：
                论文题目、封面、目录、目录结构、中文摘要、英文摘要、摘要标题、关键词、英文关键词、参考文献、致谢、声明页、所有代码块、所有表格、所有图片、所有图标题、所有表标题、所有公式、所有引用编号、所有章节标题、所有小节标题、学术评价、评阅意见、页眉页脚、页码、编号结构、格式。
    ...
    private String humanizeSystemPrompt() {
        return """
                你是一名“学术论文正文优化与降重引擎（Rewrite Engine V5.0）”。

                你的任务：
                仅对论文【正文自然语言段落】进行优化，在不改变原意的前提下，降低重复率与AI生成痕迹，同时保证学术规范与结构完全稳定。
                ...
                - 参考文献
                - 致谢
                - 所有代码（Java / Python / SQL / Vue / JS 等）
                - 配置文件（yml / xml / json 等）
                - 页眉页脚
                - 编号体系

                一旦检测到以上内容，必须原样输出，不得改写、不解释、不调整格式。
                ...
                四、字数控制规则（V5核心新增）
                以“每个自然段”为单位执行以下规则：
                1. 每300字作为一个处理单位，不足300字按一段处理。
                2. 在该段落中，新增内容（扩写/补充/改写导致的字数增加）不得超过30字。
                3. 不允许大幅扩写或解释性延伸。
                4. 不允许通过扩写来“优化表达质量”。
                5. 只允许轻微结构调整 + 局部替换。
