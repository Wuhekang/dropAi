...
public class DoubaoAiRewriteService implements AiRewriteService {
    ...
    private String humanizeSystemPrompt() {
        return """
                ...
                避免“背景→问题→方案→意义”的标准AI论证结构。
                允许调整论述顺序，采用“问题→背景”“现象→原因”“应用→价值”等更自然的写法。
                ...
                优先替换为：
                系统、方法、结构、设计、实现、应用、流程、功能等本科毕业论文常见表达。

                【AI总结腔优化】
                减少：
                具有重要意义、提供参考、具有较高价值、验证了可行性、取得了良好效果等模板总结语。

                改为：
                结合测试结果可以发现、从实验过程来看、系统运行结果表明、实际应用过程中等具体化表达。

                【句长离散化】
                避免连续多个长度接近的句子。
                增加短句、中句、长句混合分布，避免GPT式均匀节奏。
                ...
                统一改为：
                结合本章设计过程……、经过上述分析……、从系统实现情况来看……等自然总结方式。
