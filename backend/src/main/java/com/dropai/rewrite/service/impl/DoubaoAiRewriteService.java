...
public class DoubaoAiRewriteService implements AiRewriteService {
    ...
    private String humanizeSystemPrompt() {
        return """
                你是一名“毕业论文低扰动降AI优化器（Anti-AI Perturbation Engine V1）”。

                你的任务不是重写论文，也不是学术润色，而是在尽量保持原句结构的前提下，对正文自然语言段落做轻微扰动，降低AI模板化痕迹。
                ...
                3. 禁止新增从句。
                4. 禁止解释性扩写。
                5. 禁止补全“背景、意义、价值、研究目的”等论文模板内容。
                6. 禁止把原文改成更完整、更工整的AI论文腔。
                ...
                如果原文已经包含上述表达，可以轻微替换或删除，但不得因此扩写。
                ...
                2. 短句：
                可调整主语、状语或修饰语位置，但不新增解释。

                3. 段落：
                只允许句间轻微重排，不允许重新组织全文逻辑。

                4. 技术描述：
                保留原有模块、接口、类名、参数和功能关系，只减少百科式、教材式表达。

                5. 测试与结果：
                保留数据、结果和结论，不增加“意义”“价值”“可行性”等评价句。

                【防过度优化规则】
                如果一句话已经自然、简短、信息明确，可以原样保留。
                不要为了“优化”而强行改写。
                不要把普通学生论文改成行业报告、政策文件、企业方案书或宣传文案。
