...
public class DiagramPromptFactory {
    ...
    private String rules(DiagramType t){return switch(t){
        case FLOWCHART->"先识别题目、控制对象、起止条件和主流程顺序；标题必须来自用户主题，禁止使用“业务流程”“系统流程”等泛标题；只画一个核心过程，不拆成多个子系统；必须有且只有一个start和一个end，所有分支最终都必须能到达该end；decision必须是二元判断，且只能有两个标签不同的出口；三档及以上状态必须合并为一个处理节点，例如“按光照等级计算PWM”，不得从同一个decision拉出三条或四条分支；失败、缓存、重试等支路必须重新汇入能够到达end的主流程；持续循环必须增加“是否停止”二元判断，“否”回到采集节点，“是”进入end，回边不能代替end；“进入下一轮”“等待下一周期”“返回等待状态”只能作为end前的普通处理节点，其下一步必须是end；非end节点不得成为死节点。";
    ...
    private String limits(DiagramType t){return switch(t){
        case FLOWCHART->"只保留一个主流程，start和end也计入节点总数，6到10个节点最合适且硬性最多10个；最多2个decision；节点文字最多12个中文字符或18个英文字符；标题控制在6到18个中文字符；按“采集/判断/输出/显示/通信”等原文顺序抽象，不把每个光照档位、按键细节或异常分支展开成长链；合并步骤时不得丢失判断的任一分支、失败/重试路径和最终end。";
