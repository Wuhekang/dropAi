...
public class SchoolSchemaInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        try(Connection c=dataSource.getConnection()){
            ...
            jdbc.execute(h2 ? "CREATE TABLE IF NOT EXISTS school(id BIGINT AUTO_INCREMENT PRIMARY KEY,school_code VARCHAR(64) NOT NULL UNIQUE,school_name VARCHAR(120) NOT NULL,enabled BOOLEAN DEFAULT TRUE NOT NULL,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL)" : "CREATE TABLE IF NOT EXISTS school(id BIGINT PRIMARY KEY AUTO_INCREMENT,school_code VARCHAR(64) NOT NULL,school_name VARCHAR(120) NOT NULL,enabled TINYINT(1) NOT NULL DEFAULT 1,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY uk_school_code(school_code)) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
            ...
            if(tableExists(c,"recharge_order")) ensure(c,"recharge_order","refund_amount","DECIMAL(10,2) DEFAULT 0 NOT NULL");
