...
public record PointTransactionVO(Long id, String jobId, String featureCode, String featureName, Integer pointsChange,
                                 Integer balanceAfter, String remark, LocalDateTime createdAt) {
    public static PointTransactionVO of(PointTransaction item) {
        return new PointTransactionVO(item.getId(), item.getJobId(), item.getFeatureCode(), item.getFeatureName(),
