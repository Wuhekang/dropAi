...
public class ParametricStandardPartGeometryGenerator {
    ...
    private String inferNonStandardGeometry(String name) {
        ...
        if (containsAny(name, "履带")) return "TRACK_PARAMETRIC";
        if (containsAny(name, "滚轮", "支重轮", "驱动轮", "从动轮")) return "WHEEL_PARAMETRIC";
        if (containsAny(name, "清扫", "刷")) return "BRUSH_PARAMETRIC";
        if (containsAny(name, "磁", "吸附")) return "MAGNET_MODULE_PARAMETRIC";
        if (containsAny(name, "检测", "传感", "导轨", "滑轨")) return "SENSOR_RAIL_PARAMETRIC";
        if (containsAny(name, "机架", "支架", "底座")) return "FRAME";
        if (containsAny(name, "外壳", "防护", "罩")) return "COVER";
        return "PLATE";
    }
