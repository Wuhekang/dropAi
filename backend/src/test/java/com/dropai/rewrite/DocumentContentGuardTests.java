...
class DocumentContentGuardTests {
    ...
    @Test
    void rewritesInlineChineseAndEnglishAbstractBodies() throws Exception {
        try (XWPFDocument document = new XWPFDocument()) {
            document.createParagraph().createRun().setText("封面文字");
            document.createParagraph().createRun().setText("摘要：本文围绕平台资料核对流程展开研究。 ");
            document.createParagraph().createRun().setText("关键词：平台；资料");
            document.createParagraph().createRun().setText("Abstract: This study examines the document review workflow.");
    ...
    @Test
    void pureRewriteDoesNotExpandItsScopeToInlineAbstractBodies() throws Exception {
        try (XWPFDocument document = new XWPFDocument()) {
            document.createParagraph().createRun().setText("封面文字");
