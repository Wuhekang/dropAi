...
HEADER_ALIASES={
 "@functionmodule":DiagramType.FUNCTION_MODULE,"@function":DiagramType.FUNCTION_MODULE,
 "@flowchart":DiagramType.FLOWCHART,"@flow":DiagramType.FLOWCHART,
 "@erdiagram":DiagramType.ER_DIAGRAM,"@er":DiagramType.ER_DIAGRAM,
 "@architecturediagram":DiagramType.ARCHITECTURE,"@architecture":DiagramType.ARCHITECTURE,
 "@usecasediagram":DiagramType.USE_CASE,"@usecase":DiagramType.USE_CASE,
 "@blockdiagram":DiagramType.BLOCK_DIAGRAM,"@block":DiagramType.BLOCK_DIAGRAM,
 "@sequencediagram":DiagramType.SEQUENCE,"@sequence":DiagramType.SEQUENCE}
...
def detect_header(text):
 ...
 for i,line in enumerate(lines):
  ...
  if value and not value.startswith("#"):idx=i;token=value;break
 if idx is None or not token.startswith("@"):
  msg="未检测到图形类型头标记。请在第一条有效行填写标准标记之一。";return HeaderResult(None,None,(idx or 0)+1,text,ParseIssue((idx or 0)+1,"错误","HEADER_MISSING",msg,token or "","增加图形头标记。"))
 dtype=HEADER_ALIASES.get(token.lower())
 if not dtype:
  choices=list(CANONICAL.values());guess=difflib.get_close_matches(token,choices,n=1,cutoff=.35);suggest=f"您是否想输入：{guess[0]}" if guess else "使用七种标准头标记之一。";return HeaderResult(None,None,idx+1,text,ParseIssue(idx+1,"错误","HEADER_UNKNOWN",f"无法识别图形头标记：{token}",token,suggest))
